...
proc parseIntSafe(s: string, default: int = 0): int =
  try:
    return parseInt(s)
  except ValueError:
    return default
...
proc parseTweet(js: JsonNode; jsCard: JsonNode = newJNull()): Tweet =
  result = Tweet(
    id: js{"legacy"}{"id_str"}.getId,
    threadId: js{"legacy"}{"conversation_id_str"}.getId,
    replyId: js{"legacy"}{"in_reply_to_status_id_str"}.getId,
    text: js{"legacy"}{"full_text"}.getStr,
    time: js{"legacy"}{"created_at"}.getTime,
    hasThread: js{"legacy"}{"self_thread"}.notNull,
    available: true,
    user: User(id: js{"legacy"}{"user_id_str"}.getStr),
    stats: TweetStats(
      replies: js{"legacy"}{"reply_count"}.getInt,
      retweets: js{"legacy"}{"retweet_count"}.getInt,
      likes: js{"legacy"}{"favorite_count"}.getInt,
      quotes: js{"legacy"}{"quote_count"}.getInt,
      views: parseIntSafe(js{"view_count_info"}{"count"}.getStr),
    )
  )

  # fix for pinned threads
  if result.hasThread and result.threadId == 0:
    result.threadId = js{"legacy"}{"self_thread", "id_str"}.getId

  if "retweeted_status" in js{"legacy"}:
    result.retweet = some Tweet()
  elif js{"legacy"}{"is_quote_status"}.getBool:
    result.quote = some Tweet(id: js{"legacy"}{"quoted_status_id_str"}.getId)

  # legacy
  with rt, js{"legacy"}{"retweeted_status_id_str"}:
    result.retweet = some Tweet(id: rt.getId)
    return

  # graphql
  with rt, js{"legacy"}{"retweeted_status_result", "result"}:
  ...
  if jsCard.kind != JNull:
    ...
    if "poll" in name:
      ...
      result.card = some parseCard(jsCard, js{"legacy"}{"entities", "urls"})

  result.expandTweetEntities(js{"legacy"})

  with jsMedia, js{"legacy"}{"extended_entities", "media"}:
    for m in jsMedia:
      case m{"type"}.getStr
      of "photo":
  ...
  with jsWithheld, js{"legacy"}{"withheld_in_countries"}:
    ...
    if js{"legacy"}{"withheld_copyright"}.getBool or
